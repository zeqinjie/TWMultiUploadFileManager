//
//  TWMultiFileManager.m
//  house591
//
//  Created by zhengzeqin on 2021/7/8.
//

#import "TWMultiFileManager.h"
#import "TWMultiUploadHeader.h"
@implementation TWMultiFileManager
#pragma mark - 沙盒目录相关
+ (NSString *)homeDir {
...
+ (NSArray *)listFilesInCoustomDirectoryByDeep:(NSString*)path deepBool:(BOOL)deep{
    if (![TWMultiFileManager isExistsAtPath:path]) {
        return @[];
    }
...
+ (NSString *)moveVideoFileAtPath:(NSString *)srcPath
                      dirPathName:(NSString *)dirPathName {
    /// 将文件移动到这里
    /// 文件后缀
    NSString *suffix = srcPath.pathExtension;
    /// 移动的目标文件夹路径
    NSString *dirPath = [NSString stringWithFormat:@"%@",dirPathName];
    
    NSString *cachesDir = [TWMultiFileManager cachesDir];
    NSString *fileDirPath = [cachesDir stringByAppendingPathComponent:dirPath];
    
    if (![TWMultiFileManager isExistsAtPath:fileDirPath]) {
        [TWMultiFileManager createDirectoryAtPath:fileDirPath];
    }
    
    NSString *fileName = [NSString stringWithFormat:@"%@.%@",[TWMultiFileManager tempFileName],suffix];
    
    /// Fixed Bug : 这里的文件路劲必须为相对路径，千万不要设置为绝对路径，因为随着版本的更新和升级，绝对路径会变的 -- 切记@！
    ...
    /// 移动文件 <剪切>
    BOOL rst = [TWMultiFileManager moveItemAtPath:srcPath toPath:dstPath overwrite:YES];
    
    TWLog(@" 移动视频文件（%@） 文件路径（%@） -----  %@" , fileName , filePath, rst?@"成功":@"失败");
    return rst ? filePath : nil;
}

+ (NSString *)copyVideoFileAtPath:(NSString *)srcPath dirPathName:(NSString *)dirPathName {
    /// 将文件移动到这里
    /// 文件后缀
    NSString *suffix = srcPath.pathExtension;
    /// 移动的目标文件夹路径
    ...
    /// 移动文件 <剪切>
    BOOL rst = [TWMultiFileManager copyItemAtPath:srcPath toPath:dstPath overwrite:YES];
